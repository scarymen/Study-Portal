...
def todo(request):
    if request.method =="POST":
        ...
        if form.is_valid():
            try:
                completed=reqeust.POST['is_complete']
                if complete==on:
                    complete=True
                else:
                    complete=False
                ...
                complete=False
            todo=Todo(user=request.user,title=request.POST['title'],is_complete=complete)
            todo.save()
            messages.success(request,f"Work Added from {request.user.username} Successfully!")
    else:
        form=TodoForm()
...
def profile(request):
    ...
    context={'homeworks':homework,'todos':todos,'homework_done':homework_done,'todo_done':todo_done}
